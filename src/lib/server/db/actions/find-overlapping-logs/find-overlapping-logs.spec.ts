...
flightLogTest(
	...
	async ({ db, testUser, testAircraft, testPilot, testPlace, expect }) => {
		...
		await createTestFlightLog(db, testUser.id, new Date("2024-01-01T10:00:00Z"), new Date("2024-01-01T11:00:00Z"), deps);
...
dbTest("only returns logs for the specified user", async ({ db, expect }) => {
	...
	const [aircraft1] = await db
		...
		.returning();
	const [pilot1] = await db.insert(schema.pilot).values({ userId: user1.id, name: "Pilot 1" }).returning();
	const [place1] = await db.insert(schema.place).values({ userId: user1.id, name: "KJFK" }).returning();

	const [aircraft2] = await db
		.insert(schema.aircraft)
		.values({ userId: user2.id, registration: "N22222", model: "Piper PA-28" })
		.returning();
	const [pilot2] = await db.insert(schema.pilot).values({ userId: user2.id, name: "Pilot 2" }).returning();
	const [place2] = await db.insert(schema.place).values({ userId: user2.id, name: "KLAX" }).returning();

	// User 1's log: 10:00 - 12:00
	await createTestFlightLog(db, user1.id, new Date("2024-01-01T10:00:00Z"), new Date("2024-01-01T12:00:00Z"), {
	...
	await createTestFlightLog(db, user2.id, new Date("2024-01-01T10:00:00Z"), new Date("2024-01-01T12:00:00Z"), {
...
flightLogTest(
	...
	async ({ db, testUser, testAircraft, testPilot, testPlace, expect }) => {
		...
		// Log 1: 09:00 - 10:00
		await createTestFlightLog(db, testUser.id, new Date("2024-01-01T09:00:00Z"), new Date("2024-01-01T10:00:00Z"), deps);

		// Log 2: 10:30 - 11:30
		await createTestFlightLog(db, testUser.id, new Date("2024-01-01T10:30:00Z"), new Date("2024-01-01T11:30:00Z"), deps);

		// Log 3: 12:00 - 13:00
		await createTestFlightLog(db, testUser.id, new Date("2024-01-01T12:00:00Z"), new Date("2024-01-01T13:00:00Z"), deps);

		// Check for overlap: 09:30 - 11:00 (overlaps with logs 1 and 2)
		const overlaps = await findOverlappingLogs(db, {
			userId: testUser.id,
			departureAt: new Date("2024-01-01T09:30:00Z"),
			arrivalAt: new Date("2024-01-01T11:00:00Z"),
		});
